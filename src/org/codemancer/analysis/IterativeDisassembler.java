...
public class IterativeDisassembler {
	/** The binary image to be disassemble. */
	...
	private void disassemble(long addr, Register pc, List<Expression> links) {
		...
		// Determine address at which disassembly should stop.
		...
		while ((addr < maxAddr) || (maxAddr == 0)) {
			...
			// Resolve references and program counter.
			...
			// Remove any bits which have been disassembled, then refill the buffer.
